...
from .device import (
    BASE_UUID_FORMAT as BANLANX4_UUID_FORMAT,
...
BANLANX4_MODEL_NUMBER_SP631E: Final = 0x631E
BANLANX4_MODEL_NUMBER_SP632E: Final = 0x632E
BANLANX4_MODEL_NUMBER_SP633E: Final = 0x633E
BANLANX4_MODEL_NUMBER_SP634E: Final = 0x634E
BANLANX4_MODEL_NUMBER_SP635E: Final = 0x635E
BANLANX4_MODEL_NUMBER_SP636E: Final = 0x636E
BANLANX4_MODEL_NUMBER_SP637E: Final = 0x637E
...
##
class _CONFIG:
    """BanlanX v4 Light Type Configuration"""
...
class BanlanX4(UniledBleModel):
    ...
    def parse_notifications(
        ...
        if cfg:
            ...
            if cfg.order and len(cfg.order) > 1:
                ...
                order = self.str_if_key_in(data[31], cfg.order)
    ...
    def build_chip_order_command(
        ...
        order = self.fetch_chip_order_code(channel, value)
        return self.__encoder(0x6B, bytearray([order]))

    def fetch_chip_order_code(
        self, channel: UniledChannel, value: str | None = None
    ) -> int:
        """Return key for chip order string"""
        cfg: _CONFIG = channel.context
        if cfg is not None and cfg.order:
            value = channel.status.chip_order if value is None else value
            default = next(iter(cfg.order))
            return self.int_if_str_in(value, cfg.order, default)
        return None
    ...
    def fetch_chip_order_list(
        ...
        cfg: _CONFIG = channel.context
        if cfg is not None and cfg.order:
            return list(cfg.order.values())
        return None

    ##
    ## Helpers
    ##
    def match_light_type_config(self, light_type: int) -> dict | None:
...
class _8E_CONFIG(_8B_CONFIG):
    def __init__(self):
        super().__init__()
        self.name = "SPI - RGBCCT (2)"


class _89_CONFIG(_CONFIG):
    def __init__(self):
        super().__init__()
...
SP630E = BanlanX4(
    ...
    conf={
        ...
        0x87: _87_CONFIG(),  # 4 CH PWM RGBW - Coexist
        0x8A: _8A_CONFIG(),  # 5 CH PWM RGBCCT - Coexist
        0x82: _82_CONFIG(),  # SPI - Single Color
        0x84: _84_CONFIG(),  # SPI - CCT (1)
        0x8D: _8D_CONFIG(),  # SPI - CCT (2)
        0x86: _86_CONFIG(),  # SPI - RGB
        0x88: _88_CONFIG(),  # SPI - RGBW - Coexist
        0x8B: _8B_CONFIG(),  # SPI - RGBCCT (1) - Coexist
        0x8E: _8E_CONFIG(),  # SPI - RGBCCT (2) - Coexist
        0x89: _89_CONFIG(),  # SPI - RGB + 1 CH PWM - Coexist
        0x8C: _8C_CONFIG(),  # SPI - RGB + 2 CH PWM - Coexist
    },
)
